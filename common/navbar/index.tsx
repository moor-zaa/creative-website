                ...
                <nav className='flex size-full items-center justify-between p-4'>
                    ...
                    <div className='flex h-full items-center'>
                        <div className='hidden md:block'>
                            {navItems.map(item => <a key={item} href={`#${item.toLowerCase()}`} className='nav-hover-btn'>{item}</a>)}
                        </div>
                        <button onClick={toggleAudioIndicator} className='ml-10 flex items-center space-x-0.5'>
                            ...
                            {[1, 2, 3, 4].map(bar => <div key={bar} className={`indicator-line cursor-pointer ${isIndicatorActive ? 'active' : ''}`} style={{ animationDelay: `${bar * 0.1}s` }} />)}
                        </button>
                    </div>
                </nav>
            </header>
